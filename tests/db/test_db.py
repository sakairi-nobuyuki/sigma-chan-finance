...
class TestDataBase:
    ...
    def test_start_session(self):
        ...
        res = InferenceResultsModel(id=1, type=type, name=name, value=value, source=source)
        ...
        res = InferenceResultsModel(id=2, type=type, name=name, value=value, source=source)
        res.print_model_by_id(res.id)
        session.add(instance=res)
        session.commit()
        session.close()
